'''
py(파이썬 스크립트)와 ipyng(주피터 노트북) 파일의 차이점
...
def demo_import_styles():
    ...
    from mymodule import add as a, multiply as m # 특정 요소에 별칭을 부여하며 import
    print(a(10, 20))
    print(m(30, 40))
...
def demo_namespace_collision():
    ...
    2) Global namespqce : 현재 파일(.py)의 전역 이름 공간
    ...
    # from-import는 현재 파일 네임스페이스로 이름을 가져오므로 충돌이 날 수 있다.
    ...
    print(calculator("hell0"))

# 표준 라이브러리 맛보기
def demo_stdlib_modules():
    '''
    표준 라이브러리란?
    파이썬을 설치하면 기본으로 함께 제공되는 공식 모듈/패키지 묶음이다.
    별도의 설치없이 바로 import 해서 사용할 수 있다.
    '''
    # math : 수학 상수/함수 제공
    import math
    ...
    # json은 api 통신/설정 파일/로그 등에 자주 사용되는 양식이다.
    import json
    ...
    payload = {"name": "hong", "score": 95, "tags": ["python", "stdlib"]}
...
def demo_package_import():
    # 전체 경로 import
    import myproject.utils.calculator
    print(myproject.utils.calculator.VERSION)

    # from-import로 간결하게
    from myproject.utils import calculator
    print(calculator.VERSION)

    # 모듈 내부 요소 직접 import
    from myproject.utils.calculator import add, multiply
    print(add(1, 2))
    print(multiply(3, 4))

'''
1) 파이썬 파이을 직접 실행하면 __name__ 값은 "__main__"으로 설정된다.
지금 실행의 시작점이 이 파일이다 라는 뜻이다.
2) 파이썬 파일을 import 해서 사용하면 __name__ 값이 "파일명"으로 설정된다.
=> 파일이 import 되었을 때 실행용 코드가 자동으로 호출되면 안되므로 부여하는 조건
(import 했는데 갑자기 print() 함수가 호출되거나 네트워크 호출이 발생하면 곤란)
'''
